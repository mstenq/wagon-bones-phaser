...
describe('UserStats', () => {
  ...
  test('normalizeStoredBeatLevel clamps invalid values', () => {
    expect(normalizeStoredBeatLevel(-1)).toBe(0);

    expect(normalizeStoredBeatLevel(99)).toBe(8);

    expect(normalizeStoredBeatLevel('bad')).toBe(0);
  });

  test('normalizeUserStatsData sanitizes corrupt stored stats', () => {
    const normalized = normalizeUserStatsData({
  ...
  test('recordEquipmentVictory stores highest difficulty per item', () => {
    recordEquipmentVictory(['horseshoe', 'dynamite'], 3);

    expect(getEquipmentHighestDifficultyBeaten('horseshoe')).toBe(3);

    expect(getEquipmentHighestDifficultyBeaten('dynamite')).toBe(3);

    expect(getEquipmentHighestDifficultyBeaten('unknown_item')).toBe(0);
  });

  test('recordEquipmentVictory does not downgrade', () => {
    recordEquipmentVictory(['horseshoe'], 5);

    recordEquipmentVictory(['horseshoe'], 2);

    expect(getEquipmentHighestDifficultyBeaten('horseshoe')).toBe(5);
  });

  test('recordEquipmentVictory upgrades to higher difficulty', () => {
    recordEquipmentVictory(['horseshoe'], 2);

    recordEquipmentVictory(['horseshoe'], 8);

    expect(getEquipmentHighestDifficultyBeaten('horseshoe')).toBe(8);
  });

  test('recordEquipmentVictory ignores empty id list', () => {
