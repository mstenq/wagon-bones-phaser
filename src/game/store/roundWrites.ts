...
/** Reroll-lock selection during ROLL (right-click lock — keeps face on reroll). */
export function setRerollLockedDice(dice: Die[]): void {
  const round = getRoundState();
  if (!round) return;
  ensureDiceInRun(dice);
  roundActions.patch({
    rerollLockedDiceIds: dice.map((d) => d.id),
    dieValuesByDieId: syncDieValuesFromDice(round.dieValuesByDieId, dice),
  });
}
...
export function removeDestroyedDiceFromRound(destroyedIds: Set<string>): void {
  ...
  if (round.phase === 'ROLL') {
    ...
    setRerollLockedDice(selectRerollLockedDice().filter((d) => !destroyedIds.has(d.id)));
    return;
  }
